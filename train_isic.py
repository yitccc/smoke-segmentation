...
from lib.EGNL_FAT import FAT_S
...
def train(train_loader, model, optimizer, epoch, best_loss):
    ...
    if (epoch+1) % 1 == 0:
        ...
        if meanloss < best_loss:
            ...
            best_loss = meanloss
            torch.save(model.state_dict(), save_path + 'FAT-%d.pth' % epoch)
            print('[Saving Snapshot:]', save_path + 'FAT-%d.pth'% epoch)
    return best_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_save', type=str, default='FAT_S')
    ...
    # ---- build models ----
    model = FAT_S(pretrained=True).cuda()
