...
def laplacian_and_add(input_feature_map):
    ...
    # 输入特征图与拉普拉斯卷积结果进行Element-wise Add操作
...
class FAT_S(nn.Module):
    ...
    def forward(self, imgs, labels=None):
        ...
        eb0 = F.interpolate(x_u_2, size=(192, 256), mode='nearest')  # [2, 128, 256, 256]
        eb0 = self.conv_output1(eb0)  # [2, 64, 256, 256])
        concatenated_features0 = torch.cat([eb0, cf0], dim=1)  # [2, 128, 256, 256]
        ...
        eb1 = F.interpolate(x_u_1, size=(192, 256), mode='nearest')  # [2, 128, 256, 256]
        eb1 = self.conv_output2(eb1)  # [2, 64, 256, 256])
        concatenated_features1 = torch.cat([eb1, cf1], dim=1)  # [2, 128, 256, 256]
        ...
        cf2 = self.conv_output1(cf2)  # [2, 64, 512, 512]

        x_u = self.resnet.layer3(x_u_1)
        x_u = self.drop(x_u)
        eb2 = F.interpolate(x_u, size=(192, 256), mode='nearest')  # [2, 256, 512, 512]
        eb2 = self.conv_output3(eb2)  # [2, 64, 512, 512]
        concatenated_features2 = torch.cat([eb2, cf2], dim=1)  # [2, 128, 512, 512]
        lm2 = self.conv_output2(concatenated_features2)  # [2, 64, 512, 512]
